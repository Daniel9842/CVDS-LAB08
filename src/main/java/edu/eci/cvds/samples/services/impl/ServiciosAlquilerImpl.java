...
public class ServiciosAlquilerImpl implements ServiciosAlquiler {
   ...
   @Override
    ...
    public void registrarAlquilerCliente(Date date, long docu, Item item, int numdias) throws ExcepcionServiciosAlquiler {
        ...
        if(consultarCliente(docu)==null){
			throw new ExcepcionServiciosAlquiler("El cliente no esta registrado");
		}
        if(consultarItem(item.getId())==null){
			throw new ExcepcionServiciosAlquiler("El item no esta registrado");
		}
    ...
    @Override
    @Transactional
    public void vetar(long idCl, boolean vetado) throws ExcepcionServiciosAlquiler {
        try {
            clienteDAO.vetar(idCl, vetado?1:0);
        } catch (PersistenceException e) {
            throw new ExcepcionServiciosAlquiler("Error al actualizar la informacion del cliente" + idCl,e);
        }
    }
}